...
async def sendmsg_handler(websocket, msg_id: int, msg_body: str, msg_expect=None, retry=True, timeout=20):
    try:
        ...
        while True:
            ...
            if msg_expect == None or (reps_Msgid == msg_expect[0] and reps_Msgbody['reason'] == msg_expect[1]):
        ...
        logging.error(f"原因: 等待 MsgId [ {reps_Msgid} ] Time out!!")
    except Exception as e:
        logging.error(f"發送WebsocketAPi錯誤: {e}", exc_info=True)
    return False, None
...
async def handle_913(websocket, msg_body, user_id, catch_data):
    try:
        if isinstance(msg_body, list) and len(msg_body) > 0:
            ...
            if cards:
                ...
                logging.info(f">>> Player {user_id} 的手牌是: {card_readable}")

    except Exception as e:
        logging.warning(f"MsgId 913 錯誤: {e}", exc_info=True)
        # send heart-beat
    try:
        heartbeat_request = {
            "msgId": 202,
        ...
        logging.warning(f"MsgId 202 錯誤: {e}", exc_info=True)
    await asyncio.sleep(0.1)
# NLH, MTT 手牌操作
@register_handler(217)
async def handle_217(websocket, msg_body, user_id, catch_data):
    try:
        #牌力邏輯
        if msg_body.get('userId') == user_id:
            points = user_points.get(user_id)
            if not points or len(points) < 2:
                logging.warning(f"[217] Player {user_id} 還沒拿好手牌，skip this action")
                return
            ...
            logging.info(f">>> Player {user_id} 的行動: {actName}")

            await websocket.send(json.dumps(msg))
    except Exception as e:
        logging.warning(f"MsgId 217 錯誤: {e}", exc_info=True)
# 公共牌
@register_handler(219)
async def handle_219(websocket, msg_body, user_id, catch_data):
    try:
        ...
        logging.warning(f"MsgId 219 錯誤: {e}", exc_info=True)
# 
@register_handler(221)
async def handle_221(websocket, msg_body, user_id, catch_data):
    try:
        ...
        logging.warning(f"MsgId 221 錯誤: {e}", exc_info=True)
#
@register_handler(216)
async def handle_216(websocket, msg_body, user_id, catch_data):
    try:
        # 公共牌信息
        ...
        logging.warning(f"MsgId 216 錯誤: {e}", exc_info=True)
#
@register_handler(219)
async def handle_219(websocket, msg_body, user_id, catch_data):
    try:
...
async def handle_218(websocket, msg_body, user_id, catch_data):
    try:
        ...
        logging.warning(f"MsgId 218 錯誤: {e}", exc_info=True)
#Rebuy
@register_handler(356)
async def handle_356(websocket, msg_body, user_id, catch_data):
    try:
...
async def handle_2122(websocket, msg_body, user_id, catch_data):
    try:
        logging.info(msg_body)
        game_id = catch_data['game_id']
        msgid_bet = 2135
        ...
        if repsStatus:
            logging.info(f"Player {user_id} 操作[確認下注]成功")
        else:
            logging.warning(f"Player {user_id} 操作[確認下注]失敗")
            
    except Exception as e:
        logging.warning(f"MsgId 2135 錯誤: {e}", exc_info=True)


# 工廠化所有機器人的各種操作函式，日後好維護

async def robot_play(websocket, user_id, catch_data):
    while True:
        try:
            ...
            logging.info(f"機器人解析RECV時遇到錯誤:{e}")
        handler_Action = MSG_Handlerlist.get(msg_id, None)
        if handler_Action:
            await handler_Action(websocket, msg_body, user_id, catch_data)
        else:
            logging.debug(f"未處理過的 MsgId: {msg_id}")
...
async def robot_login(account, yamlData):
    ...
    game_type = yamlData.get("gametype", "")
    ...
    async with websockets.connect(
        ...
        for case in cases:
            ...
            if msg_id == 201:
                senddata["userName"] = account["Acc"]
                senddata["password"] = account["PW"]
            # 替換 game_id
            if "gameID" in senddata and catch_data.get("game_id"):
                senddata["gameID"] = catch_data["game_id"]

            # 嘗試入座類型（208, 2102）
            if msg_id in [208]:
                for pos in range(8):
                    senddata["pos"] = pos if "pos" in senddata else pos
                    state, res = await sendmsg_handler(websocket, msg_id, senddata, expect, retry)
                    if state and res['reason']:
                        logging.info(f"[入座成功] MsgId: {msg_id}, Pos: {pos}")
                        break
            if msg_id in [2102]:
                for pos in range(8):
                    senddata["seat"] = pos if "seat" in senddata else pos
                    state, res = await sendmsg_handler(websocket, msg_id, senddata["seat"], expect, retry)
                    if state:
                        logging.info(f"[入座成功] MsgId: {msg_id}, Pos: {pos}")
                        break
                    await asyncio.sleep(0.1)
            ...
            # save game_id
            if state and msg_id in [2101, 207]:
                catch_data["game_id"] = res.get("gameID")
                logging.info(f">>>即時更新game_id: {catch_data['game_id']}")
            # 登入成功時也保 userId
            if state and msg_id == 201:
                catch_data["user_id"] = res.get("userId")
                logging.info(f"{catch_data['user_id']} 已成功登入!!")
            # 單純為了console辨識用
            if state and msg_id == 213:
                jsonify_res = json.load(res.get("userInfo"))
                catch_data["nick_name"] = jsonify_res.get("nickname")

        await robot_play(websocket, catch_data["user_id"], catch_data)
